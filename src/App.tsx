...
import { STRINGS, APP_CONFIG } from './constants/config';

export default function App() {
  const [showLanding, setShowLanding] = useState(true);
  const [activeTab, setActiveTab] = useState<'papers' | 'faq' | 'full_database'>('papers');
  ...
  const filteredPapers = useMemo(() => {
    return papers.filter(paper => {
      const matchesSearch = paper.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
                            ...
                            paper.significance.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const filteredFaq = useMemo(() => {
    return faqData.filter(item => {
      ...
      const matchesCategory = selectedFaqCategory ? item.category === selectedFaqCategory : true;
      return matchesSearch && matchesCategory;
    });
  }, [faqData, searchQuery, selectedFaqCategory]);

  const handleTabChange = (tab: 'papers' | 'faq' | 'full_database', category?: string) => {
    setActiveTab(tab);
            ...
            {activeTab === 'papers' ? (
                ...
                <aside className="lg:w-64 flex-shrink-0 space-y-6">
                  <div>
                    <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                      <Filter className="w-3 h-3" /> {STRINGS.APP.FILTER_TITLE}
                    </h2>
                    <div className="flex flex-wrap lg:flex-col gap-2">
                      ...
                      >
                        <span>{STRINGS.APP.ALL_PAPERS}</span>
                ...
                <section className="flex-1">
                  <div className="mb-8 flex items-center justify-between">
                    <h2 className="text-4xl font-display uppercase tracking-tighter">
                      {selectedCategory || STRINGS.APP.ALL_PAPERS}
                      <span className="text-sm font-bold text-slate-400 ml-4 tracking-widest">[{filteredPapers.length} {STRINGS.APP.ITEMS_SUFFIX}]</span>
                    </h2>
                  </div>
                        ...
                        >
                          <div className="relative z-10">
                            ...
                            <div className="flex items-center justify-end">
                              <a 
                                href={`${APP_CONFIG.CNKI_BASE_URL}${encodeURIComponent(paper.title)}`}
                                target="_blank"
                              ...
                              >
                                {STRINGS.APP.READ_MORE} <ChevronRight className="w-3 h-3" />
                              </a>
                            </div>
                          </div>
                        </motion.div>
                  ...
                  {filteredPapers.length === 0 && (
                    <div className="py-20 text-center">
                      ...
                      <h3 className="text-2xl font-bold uppercase tracking-widest">{STRINGS.APP.NO_RESULTS}</h3>
                      <p className="text-slate-500 font-bold mt-2">{STRINGS.APP.TRY_DIFFERENT}</p>
                    </div>
                  )}
                </section>
              </div>
              ...
              <div className="max-w-4xl mx-auto">
                <div className="mb-12 text-center">
                  <h2 className="text-5xl font-display uppercase tracking-tighter mb-4">{STRINGS.APP.FAQ_TITLE}</h2>
                  <p className="text-lg font-bold text-slate-500 uppercase tracking-widest">{STRINGS.APP.FAQ_SUBTITLE}</p>
                </div>
              ...
              <div className="w-full overflow-hidden">
                <div className="mb-12 text-center">
                  <h2 className="text-5xl font-display uppercase tracking-tighter mb-4">{STRINGS.APP.FULL_DB_TITLE}</h2>
                  <p className="text-lg font-bold text-slate-500 uppercase tracking-widest">{STRINGS.APP.FULL_DB_SUBTITLE}</p>
                </div>
                ...
                {isSearchingFull ? (
                  <div className="py-20 text-center">
                    ...
                    <p className="mt-4 font-mono text-sm text-slate-400 uppercase tracking-widest">{STRINGS.APP.QUERYING}</p>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-[var(--color-brutal-black)] text-white font-mono text-xs uppercase tracking-widest">
                          <th className="px-6 py-4 border-r border-white/10">{STRINGS.APP.TABLE_HEADERS.YEAR}</th>
                          <th className="px-6 py-4 border-r border-white/10">{STRINGS.APP.TABLE_HEADERS.TITLE}</th>
                          <th className="px-6 py-4 border-r border-white/10">{STRINGS.APP.TABLE_HEADERS.AUTHOR}</th>
                          <th className="px-6 py-4 border-r border-white/10">{STRINGS.APP.TABLE_HEADERS.INSTITUTION}</th>
                          <th className="px-6 py-4">{STRINGS.APP.TABLE_HEADERS.DATABASE}</th>
                        </tr>
                      </thead>
                ...
                {!isSearchingFull && totalFullPapers > 0 && (
                  <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-6">
                    <div className="text-xs font-mono font-bold text-slate-400 uppercase tracking-widest">
                      {STRINGS.APP.TOTAL_RECORDS.replace('{total}', totalFullPapers.toString())}
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      >
                        {STRINGS.APP.NEXT_PAGE}
                      </button>
                    </div>
                    
                    <div className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest">
                      {STRINGS.APP.POWERED_BY}
                    </div>
                  </div>
                )}
              </div>
